...
export default function SynthesisAltar({ stats, onSynthesize, onClose }: SynthesisAltarProps) {
  ...
  const [selectedArtifactId, setSelectedArtifactId] = useState<string | null>(
    synthesisList[0]?.id || null,
  );

  const selectedArtifact = synthesisList.find((r) => r.id === selectedArtifactId);
  const isUnlocked = selectedArtifactId
    ? stats.unlockedResearchIds.includes(selectedArtifactId)
    : false;
      ...
      <div className="relative w-full h-full max-w-6xl bg-[#0a0a0b] border border-orange-950/30 rounded-[2.5rem] shadow-[0_0_100px_rgba(249,115,22,0.15)] flex flex-col overflow-hidden">
        ...
        <div className="flex justify-between items-center p-8 z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl font-black text-orange-500 tracking-tighter uppercase italic">
                Synthesis Altar
              </h2>
              <p className="text-[10px] text-zinc-500 font-bold tracking-[0.3em] uppercase opacity-60 mt-1">
                Sacred Relic Forging Station
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-3">
            {synthesisList.map((artifact) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-14 h-14 rounded-xl flex items-center justify-center text-3xl shrink-0 transition-transform duration-500 group-hover:scale-110
                  ...
                  <div className="flex-1 text-left">
                    <h3
                      className={`text-sm font-black tracking-tight ${active ? 'text-orange-400' : 'text-zinc-300'}`}
                    >
                      {artifact.nameKo}
                    </h3>
                    <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-wider">
                      {artifact.bonus
                        ? `${artifact.bonus.stat} +${artifact.bonus.value}`
                        : 'Unique Growth Effect'}
                    </p>
                  </div>
            ...
            {selectedArtifact ? (
              <div className="w-full h-full flex flex-col items-center animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                <div className="w-48 h-48 bg-[#0a0a0b] border border-zinc-900 rounded-full flex items-center justify-center relative mb-8 group">
                  ...
                  <div
                    className={`text-8xl p-8 rounded-full transition-transform duration-700 group-hover:scale-110 z-10 ${isUnlocked ? 'filter-none' : 'grayscale brightness-50'}`}
                  >
                    💎
                  </div>
                ...
                <div className="w-full flex flex-col gap-4 mb-8">
                  ...
                  {selectedArtifact.effectDescriptionKo && (
                    <div className="flex flex-col items-center bg-zinc-900 p-4 rounded-xl border border-emerald-900/30">
                      <span className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-2">
                        Growth Effect
                      </span>
                ...
                <div className="w-full space-y-3 mb-10 flex-1">
                  <p className="text-[10px] font-black text-zinc-700 uppercase tracking-[0.2em] mb-4 text-center">
                    Synthesis Materials
                  </p>
                  {selectedArtifact.requirements &&
                    Object.entries(selectedArtifact.requirements).map(([res, amount]) => {
                      const owned =
                        res === 'goldCoins' ? stats.goldCoins : (stats.inventory as any)[res] || 0;
                      const met = owned >= (amount as number);
                      return (
                        <div
                          key={res}
                          className="flex justify-between items-center px-4 py-3 bg-zinc-900/30 rounded-xl border border-zinc-900/50"
                        >
                          <div className="flex items-center gap-3">
                            {res === 'goldCoins' ? (
                              <AtlasIcon name="GoldIcon" size={16} />
                            ) : (
                              <div className="w-4 h-4 bg-zinc-800 rounded-sm flex items-center justify-center text-[10px]">
                                📦
                              </div>
                            )}
                            <span className="text-xs font-bold text-zinc-500 capitalize">
                              {res === 'goldCoins' ? 'Gold Coins' : res}
                            </span>
                          </div>
                          <span
                            className={`text-xs font-black tabular-nums ${met ? 'text-zinc-300' : 'text-rose-500 animate-pulse'}`}
                          >
                ...
                {isUnlocked ? (
                  ...
                  >
                    <span className="group-hover:rotate-12 inline-block transition-transform">
                      🔥
                    </span>{' '}
                    INITIATE SYNTHESIS
                  </button>
