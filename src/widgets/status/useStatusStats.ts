...
export function useStatusStats(stats: PlayerStats): StatusStatsResult {
  return useMemo(() => {
    ...
    const totalSpeedBonusMult = Math.min(0.9, (artifactBonuses.miningSpeed || 0) + runeSpeedBonus + masteryBonuses.miningSpeedMult);
    ...
    const baseSpeedMult = ((baseSpeedStat + artifactSpeedBonus) / 100) * (1 + masteryBonuses.moveSpeedMult);
    ...
    const statBreakdowns: Record<string, StatBreakdownItem[]> = {
      ...
      critRate: [
        ...
        { label: 'Artifact', value: `+${((artifactBonuses.critRate || 0) * 100).toFixed(1)}%`, color: 'text-orange-400' },
      ],
      moveSpeed: [
        ...
        { label: 'Artifact', value: `+${artifactSpeedBonus.toFixed(0)}%`, color: 'text-orange-400' },
      ...
      critDmg: [
        ...
        { label: 'Artifact', value: `+${((artifactBonuses.critDamage || 0) * 100).toFixed(0)}%`, color: 'text-orange-400' },
      ],
      miningSpeed: [
      ...
      luck: [
        ...
        { label: 'Artifact', value: `+${(artifactBonuses.luck * 100).toFixed(0)}`, color: 'text-orange-400' },
      ]
    };
