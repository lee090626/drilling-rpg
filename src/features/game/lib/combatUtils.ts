 ...
 * 두 좌표 사이의 거리를 계산합니다.
 */
export const getDistance = (p1: Position | { x: number, y: number }, p2: Position | { x: number, y: number }) => {
  const dx = p1.x - p2.x;
  const dy = p1.y - p2.y;
 ...
 */
export const getEntitiesInRange = (entities: Entity[], pos: { x: number, y: number }, range: number) => {
